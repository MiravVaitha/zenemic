...
Rules:
- Infer sensible values, but never invent specifics that aren't implied. If something is genuinely unknown, return null for that field. "dateLabel", "timeLabel", "locations", "startsAtISO", "endsAtISO" and "budgetMajor" all accept null — use it. The host is shown every field to confirm before anything is created, and a null becomes a question they answer. A guess becomes a wrong answer they may not notice, so null is always the better choice when the message doesn't say.
...
- "title" should be a short, human event title (e.g. "Mira's 28th Birthday", "Q3 Team Offsite"). Do not include the date in the title. This one is never null — summarise whatever the message is about.
- "dateLabel" is a display string like "07 Jun 2026" or "14-16 Jun 2026". "timeLabel" like "7:30 PM", "Fri 4 PM", or "All day". Return null for either when the message gives nothing to resolve it from — "soon", "sometime next month" and "TBC" are not dates. Do not fall back to today's date.
